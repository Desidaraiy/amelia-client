...
class ProductOrderItemWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    print("${this.order.delivery_timestamp.difference(DateTime.now()).inMinutes}");
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: this.order.delivery_timestamp.difference(DateTime.now()).inMinutes <=
              120 && this.order.delivery_timestamp.difference(DateTime.now()).inMinutes >= 0
              ? Theme.of(context).focusColor.withOpacity(0.0)
