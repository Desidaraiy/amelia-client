...
class NotificationController extends ControllerMVC {
  List<model.Notification> newNotifications = [];
  List<model.Notification> oldNotifications = [];
    ...
    stream.listen((model.Notification _notification) {
      setState(() {
        if (_notification != null && !_notification.read)
          newNotifications.add(_notification);
        if (_notification != null && _notification.read)
          oldNotifications.add(_notification);
      });
    }, onError: (a) {
      print(a);
  ...
  void doRemoveNotification(model.Notification _notification) async {
    removeNotification(_notification).then((value) {
      setState(() {
        ...
        this.newNotifications.remove(_notification);
      });
      ScaffoldMessenger.of(scaffoldKey?.currentContext).showSnackBar(SnackBar(
